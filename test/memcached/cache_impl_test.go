...
func TestMemcached(t *testing.T) {
	...
	client.EXPECT().Increment(
		"domain_key2_value2_subkey2_subvalue2_1200", uint64(1),
	...
	limits = []*config.RateLimit{
		nil,
		config.NewRateLimit(
			10, pb.RateLimitResponse_RateLimit_MINUTE, sm.NewStats("key2_value2_subkey2_subvalue2"),
	...
	client.EXPECT().Increment(
		"domain_key3_value3_subkey3_subvalue3_950400", uint64(2),
	...
	limits = []*config.RateLimit{
		...
		config.NewRateLimit(
			10, pb.RateLimitResponse_RateLimit_DAY, sm.NewStats("key3_value3_subkey3_subvalue3"),
			false, false, "", nil, false,
		),
	}
	assert.Equal(
...
func TestMemcachedGetError(t *testing.T) {
	...
	limits = []*config.RateLimit{
		config.NewRateLimit(
			10, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key_value1"), false, false, "",
			nil, false,
		),
	}
...
func testLocalCacheStats(
	localCacheStats stats.StatGenerator, statsStore stats.Store, sink *common.TestStatSink,
...
func TestMemcacheWithJitter(t *testing.T) {
	...
	statsStore := stats.NewStore(stats.NewNullSink(), false)
	sm := mockstats.NewMockStatManager(statsStore)
	cache := memcached.NewRateLimitCacheImpl(
		client, timeSource, rand.New(jitterSource), 3600, nil, sm, 0.8, "",
	)
...
func TestMemcacheAdd(t *testing.T) {
	...
	limits = []*config.RateLimit{
		config.NewRateLimit(
			10, pb.RateLimitResponse_RateLimit_MINUTE, sm.NewStats("key2_value2"), false, false, "",
			nil, false,
		),
	}
	...
	assert.Panics(
		func() {
			memcached.NewRateLimitCacheImplFromSettings(
				s, timeSource, nil, nil, statsStore, mockstats.NewMockStatManager(statsStore),
			)
		},
	)
}
