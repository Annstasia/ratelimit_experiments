...
func TestGenerateCacheKeys(t *testing.T) {
	...
	baseRateLimit := limiter.NewBaseRateLimit(
		timeSource, rand.New(jitterSource), 3600, nil, 0.8, "", sm,
	)
...
func TestGenerateCacheKeysPrefix(t *testing.T) {
	...
	timeSource.EXPECT().UnixNow().Return(int64(1234))
	baseRateLimit := limiter.NewBaseRateLimit(
		timeSource, rand.New(jitterSource), 3600, nil, 0.8, "prefix:", sm,
	)
	request := common.NewRateLimitRequest("domain", [][][2]string{{{"key", "value"}}}, 1)
	limits := []*config.RateLimit{
		config.NewRateLimit(
...
func TestNoOverLimitWithLocalCache(t *testing.T) {
	...
	assert.Equal(
		false, baseRateLimitWithLocalCache.IsOverLimitWithLocalCache("domain_key_value_1234"),
	)
}
...
func TestGetResponseStatusOverLimitWithLocalCache(t *testing.T) {
	...
	limits := []*config.RateLimit{
		config.NewRateLimit(
			5, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key_value"), false, false, "",
			nil, false,
		),
	}
...
func TestGetResponseStatusOverLimitWithLocalCacheShadowMode(t *testing.T) {
	...
	limits := []*config.RateLimit{
		config.NewRateLimit(
			5, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key_value"), false, true, "",
			nil, false,
		),
	}
	limitInfo := limiter.NewRateLimitInfo(limits[0], 2, 6, 4, 5)
...
func TestGetResponseStatusBelowLimitShadowMode(t *testing.T) {
	...
	limits := []*config.RateLimit{
		config.NewRateLimit(
			10, pb.RateLimitResponse_RateLimit_SECOND, sm.NewStats("key_value"), false, true, "",
			nil, false,
		),
	}
	limitInfo := limiter.NewRateLimitInfo(limits[0], 2, 6, 9, 10)
