...
func NewRunner(s settings.Settings) Runner {
	...
	case s.UseStatsd:
		...
		store = gostats.NewStore(
			gostats.NewTCPStatsdSink(
				gostats.WithStatsdHost(s.StatsdHost), gostats.WithStatsdPort(s.StatsdPort),
			), false,
		)
...
func createLimiter(
	srv server.Server, s settings.Settings, localCache *freecache.Cache, statsManager stats.Manager,
) (limiter.RateLimitCache, io.Closer) {
...
func (runner *Runner) Run() {
	...
	serverReporter := metrics.NewServerReporter(
		runner.statsManager.GetStatsStore().ScopeWithTags(
			"ratelimit_server", s.ExtraTags,
		),
	)

	srv := server.NewServer(
		s, "ratelimit", runner.statsManager, localCache,
		settings.GrpcUnaryInterceptor(serverReporter.UnaryServerInterceptor()),
	)
	runner.mu.Lock()
	runner.srv = srv
	runner.mu.Unlock()
